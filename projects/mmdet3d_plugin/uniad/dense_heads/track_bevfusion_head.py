...
class BEVFusionTrackHead(DETRHead):
    """Head of Detr3D.
    Args:
    ...
    def __init__(self,
        ...
        super(BEVFusionTrackHead, self).__init__(
    ...
    def get_bev_pos(self, dtype, B=1):
        bev_queries = self.bev_embedding.weight.to(dtype)
        bev_mask = torch.zeros((B, self.bev_h, self.bev_w),device=bev_queries.device).to(dtype)
        bev_pos = self.positional_encoding(bev_mask).to(dtype)
    ...
    def get_detections(
        ...
        for lvl in range(hs.shape[0]):
            ...
            # # debug
            # print('lvl outputs_class: ', lvl, outputs_class)
